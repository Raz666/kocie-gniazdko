...
(async()=>{
 ...
 await page.goto('file:///'+path.join(__dirname,'preview-qa.html').replaceAll('\\','/'));
 ...
 await page.screenshot({path:path.join(__dirname,'first-screen.png')});
 ...
 await root.screenshot({path:path.join(__dirname,'kocie-gniazdko-mobile-full.png')});
